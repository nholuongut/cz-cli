 ...
 * a previous nholuongut adapter installed or if the proper fields were
 * provided. It defers that responsibility to init.
 */

/**
 * Modifies the package.json, sets config.nholuongut.path to the path of the adapter
 * Must be passed an absolute path to the cli's root
 */
function addPathToAdapterConfig (cliPath, repoPath, adapterNpmName) {

  let nholuongutAdapterConfig = {
    config: {
      nholuongut: {
  ...
  if (_.get(packageJsonContent, 'config.nholuongut.path') !== adapterNpmName) {
    newPackageJsonContent = _.merge(packageJsonContent, nholuongutAdapterConfig);
  }
