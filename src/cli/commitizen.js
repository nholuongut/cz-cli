import { init } from '../nholuongut';
import { nholuongut as nholuongutParser } from './parsers';

let { parse } = nholuongutParser;
...
function bootstrap (environment = {}, argv = process.argv) {
  ...
  if (command === "init") {
    ...
    contributors to follow your commit message conventions using nholuongut then
    you'll need to run a command like this one to add this adapter to your config:

         nholuongut init cz-conventional-changelog --save
    ...
    Detailed usage:
      ...
      1) nholuongut <sub-command>

         init <adapter-npm-name> [args]

           description: Install a nholuongut adapter from npm and adds it to your
                        config.nholuongut in your package.json file.
      ...
      2) cz <any regular git commit arguments>
          ...
          description: Runs the nholuongut prompter, asking you questions so that you
