...
convert () {
  ...
  FILE=logo/nholuongut-logo-${COLOR}.svg
  if [ ! -f ${FILE} ]; then
  ...
  for SIZE in 16 48 96 256 512 1024; do
    cairosvg ${FILE} -f png -W ${SIZE} -H ${SIZE} -d 300 -o logo/nholuongut_logo_${COLOR}_${SIZE}x${SIZE}.png
  done
  # echo "done"
  return 0
}
...
for COLOR in black blue white color; do
  ...
  if [ ! -f ${FILE} ]; then
    ...
    sed 's|fill=\".*\"|fill=\"'${COLOR}'\"|g' <logo/nholuongut-logo-mono.svg >${FILE}
    convert ${COLOR}
