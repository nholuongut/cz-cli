...
import { commit as nholuongutCommit, init as nholuongutInit } from '../../src/nholuongut';
...
describe('commit', function () {
  ...
  it('should commit simple messages', function (done) {
    ...
    // version of prompter.
    nholuongutCommit(inquirer, repoConfig.path, prompter, { disableAppendPaths: true, quiet: true, emitData: true }, function () {
...
${(os.platform === 'win32') ? '' : '    '}
    ...
    // Pass in inquirer but it never gets used since we've mocked out a different
    // version of prompter.
    nholuongutCommit(inquirer, repoConfig.path, prompter, { disableAppendPaths: true, quiet: true }, function () {
      log(repoConfig.path, function (logOutput) {
        expect(logOutput).to.have.string(dummyCommitMessage);
        done();
      });
  ...
  it('should save directly to .git/COMMIT_EDITMSG with --hook option', function (done) {
    ...
    nholuongutCommit(inquirer, repoConfig.path, prompter, { disableAppendPaths: true, quiet: true, emitData: true, hookMode: true }, function (err) {
  ...
  it('should throw error if staging area is empty', function (done) {
    ...
    nholuongutCommit(inquirer, repoConfig.path, prompter, { disableAppendPaths: true, quiet: true, emitData: true }, function (error) {
...
function quickPrompterSetup (repoConfig, adapterConfig, commitMessage, options = {}) {
  ...
  nholuongutInit(repoConfig.path, adapterConfig.npmName);
